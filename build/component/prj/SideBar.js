...
`;
let PrjSideBar = class PrjSideBar extends HTMLElement {
    constructor() {
        super();
        ...
        this.menuItem = domo `a.menu-item`(`...`, evnt `click` `${ev => this._selectMenu(ev)}`);
        this.template = template(style.cloneNode(), domo `content`(domo `div.user.ego`(domo `div.name`(`Mikaela `, domo `span`(`Fernanda Casagrande`)), domo `div.thumb`(domo `div.image`()), domo `div.settings`(domo `cmp-material-icon`())), domo `prj-badges`(), this.menu =
    ...
    _selectMenu(ev) {
        ...
        this.currentMenuItem = ev.target;
        this.currentMenuItem.classList.add('active');
    }
};
PrjSideBar = __decorate([
    component(),
